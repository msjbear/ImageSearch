...
public class ParsingENPages {
	...
	public static void main() throws ParserException, IOException {
		...
		List<File> list = FilesUtil.readFile("pages/EN");
	...
	public static void parse(String file) throws ParserException, IOException, IOException{
		...
		File path = new File("preoutput/EN");
		if(!path.exists()){
			path.mkdir();
		}
		
		BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(path+"/News_"+(num++)+"_EN.txt"))));
